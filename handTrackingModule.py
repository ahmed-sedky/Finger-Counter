    ...
    def findHands(self,img ,draw =True):
        ...
        if (self.result.multi_hand_landmarks):
            for hand in self.result.multi_handedness:
                self.L_R_hand = hand.classification[0].label
        ...
        return self.L_R_hand , img
    def findPosition(self,img ,hand_number = 0 , draw =True):
        self.landmarks_list = []
        height ,width ,channel = img.shape
        if (self.result.multi_hand_landmarks):
            myHands = self.result.multi_hand_landmarks[hand_number]
